<?php
// RiBot Add Phrase File
// Source: https://github.com/Ridys/ribot
// Dmitry Ageykin (c) 2017
error_reporting(E_ALL);
ini_set('display_errors',1);
require_once 'db.php';
mysqli_close($link);
if(isset($_POST['text_full'],$_POST['phrase1'],$_POST['phrase2'])) {
  $u_text = $_POST['text_full'];
  $u_p1 = $_POST['phrase1'];
  $u_p2 = $_POST['phrase2'];
  // замена символов
  $points = array('.', 'ё', "'", '?', '!', ',', ')', '(', ':');
  $replace = array('', 'е', "", '', '', '', '', '');
  $textr = str_replace($points, $replace, $u_text);
    require_once 'db.php';
    $query = "INSERT INTO `answer`(`text`, `1`, `2`) VALUES ('$textr','$u_p1','$u_p2')";
    if (mysqli_query($link, $query)) {
      mysqli_close($link);
      exit("Фраза добавлена. <a href=\"$homedir/adm/add.php\">Вернуться назад</a>.");
    } else {
      mysqli_close($link);
      exit("Возникла ошибка при добавлении фразы. <a href=\"$homedir/adm/add.php\">Вернуться назад</a>.");
    }
}
?>
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8" />
<link rel="stylesheet" type="text/css" href="main.css">
<title>Добавить фразу - RiBot</title><center>
<form action="<?=$homedir?>/adm/" method="post" class="search">
  <input type="search" name="search" placeholder="поиск по фразе" class="input" />
  <input type="submit" name="" value="" class="submit" />
</form><h3>Добавить новую фразу в БД</h3></center>
</head>
<body><div align="center"><form action="" method="post">
<h3><em>Скрипт-фраза</em></h3>
<textarea name="text_full" rows="5" cols="50" maxlength="255"></textarea>
<h3><em>Ответ бота #1</em></h3>
<textarea name="phrase1" rows="5" cols="50" maxlength="255"></textarea>
<h3><em>Ответ бота #2</em></h3>
<textarea name="phrase2" rows="5" cols="50" maxlength="255"></textarea>
<p><input type="submit" value="Отправить"></p>
</div>
</form>
<div id="button"> »
	<div id="s_panel">
	<a title="Главная" href="<?=$homedir?>/adm/"><img width="64px" height="64px" src="img/home.png"/></a>
	<a title="Добавить фразу" href="<?=$homedir?>/adm/add.php"><img width="64px" height="64px" src="img/add.png"/></a>
	</div>
</div>
</body>
</html>
<!--developed by Dmitry Ageykin-->